...
void Sha1Tests::sha1Tests1()
{
    ...
    QVERIFY(toHash.fromStdString(""));
    result = hashObj.hash(toHash);
    QVERIFY(memcmp((void *)result.toUCharArrayPtr(), (void *)&emptyStringResult, 20) == 0);
}
...
void Sha1Tests::sha1Tests2()
{
    ...
    QVERIFY(toHash.fromStdString("abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu"));
    result = hashObj.hash(toHash);
    ...
    QVERIFY(memcmp((void *)result.toUCharArrayPtr(), (void *)&string896bits, 20) == 0);
}
